...
const adaptConversation = (value) => {
  // The composite's submission key must not also become the host's inherited
  // data-binding key when a cached route reconnects.
  if (value === '#storeKey:storeKey') return '#submitKey:storeKey';
  if (!Array.isArray(value)) return value;
  const children = value.map(adaptConversation);
  return value.includes('@rg=cnv-scroll')
    ? [...children, '@tabindex=0', '@role=log', '@aria-label=$t.a11y.chatHistory']
    : children;
};
const chatDefs = conversationDefs.map(def => def.tag === 's-cnv' ? {
  ...def,
  props: def.props.map(prop => prop === 'storeKey' ? 'submitKey' : prop),
  children: adaptConversation(def.children),
} : def);
...
const conversationView = () => {
  ...
  view[1][':submitKey'] = view[1][':storeKey'];
  delete view[1][':storeKey'];
  return view;
};
...
const primaryAssistant = {
  tag: 'primary-assistant',
  children: [['s-cn', '@rg=primary-assistant', '@l=vertical', '@role=region', '@aria-label=$t.a11y.chatConversation', [
    ['s-cn', '@rg=chat-heading', '@l=row', [
      ['strong', '=Your next application starts here.'],
    ...
    ['p', '@rg=chat-note', '=Pragmatical AI assistant · AI-generated answers. Avoid sharing sensitive information.'],
  ]]],
  methods: { newConversation: resetConversation },
};

// Persistent shell UI. Hiding the panel leaves the native conversation mounted.
export const assistantDefs = [...chatDefs, primaryAssistant, {
  tag: 'site-assistant',
  children: [
    ['s-cn', '?!assistantPrimary', '@rg=assistant-panel', '@id=site-assistant-panel', '@role=dialog', '@aria-modal=false', '@aria-label=$t.a11y.chatConversation', ':dataField=hidden', ':dataSource=assistantHidden', '@l=vertical', [
      ['s-cn', '@rg=chat-heading', '@l=row', [
    ...
    ['s-b', '?!assistantPrimary', '@rg=assistant-launcher', '@c=pr', ':ref=launcher', '@aria-label=$assistantLauncherLabel', '@aria-controls=site-assistant-panel', '@aria-expanded=$assistantExpanded', '~cl:call:toggle', [
  ...
  callbacks: {
    connected() {
      ...
      const route = this.store.subscribe('route', (path) => {
        const primary = path === '/' || path === '/contact';
        const wasPrimary = this.store.getValue('assistantPrimary');
        this.store.setValue('assistantPrimary', primary);
        if (wasPrimary && !primary && (this.store.getValue('chatRows') || []).length > 1) this.store.setValue('assistantHidden', false);
      });
      this._subs.push({ sub: rows }, { sub: visible }, { sub: route });
    },
  },
  methods: {
