...
The homepage and contact page place the assistant directly in the main content, with the composer visible on the opening screen. Other pages provide a floating launcher that opens a large, centred non-modal dialog. Escape minimises that dialog and returns focus to its launcher. Only one conversation view is rendered at a time.

The shared marketing shell keeps `site-assistant` mounted to own the conversation state and HTTP service subscriptions. Navigating between the primary conversation and the floating panel preserves history and an in-flight response. Recent conversation history is kept in sessionStorage in the current tab, with a New conversation control to clear it. The server does not save transcripts or submit enquiries.

The UI uses SWC's `communication/conversation` composite. The application adds a declarative keyboard-accessible log region to its history and names the host's submission property `submitKey` to avoid colliding with the inherited data-binding `storeKey` on cached-route reconnection. The form still receives the native `storeKey`. The native composer publishes to the store; the declared HTTP streaming service calls `/swc/chat`. Application logic supplies bounded recent history. Responses stream through the framework's store bindings. The existing agent-stack full chat suite requires identity and storage capabilities beyond this public website assistant.
